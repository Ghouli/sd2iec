 ...
 * can receive slow or fast serial ($82CA)
 *
...
static int16_t _iec_getc(void) {
  ...
  do {                                             // $8311
    tmp = IEC_PIN & (IEC_BIT_ATN | IEC_BIT_CLOCK | IEC_BIT_SRQ);
  ...
  if (tmp == IEC_BIT_ATN) for (i=0; i<8; i++) { /* fast serial */
      ...
      do {
        tmp = IEC_PIN & (IEC_BIT_DATA | IEC_BIT_SRQ);
	  ...
	  do {
		tmp = IEC_PIN & (IEC_BIT_ATN | IEC_BIT_SRQ);
	  } while (tmp == (IEC_BIT_ATN | IEC_BIT_SRQ));
      ...
      if (iec_data.device_state == DEVICE_LISTEN) {
		...
		ATOMIC_BLOCK( ATOMIC_FORCEON ) {
		   ...
		   do { /* wait for first byte to start ($8199) */
               tmp = IEC_PIN & (IEC_BIT_ATN | IEC_BIT_CLOCK);
      ...
      if (iec_data.iecflags & COMMAND_RECVD) {
        ...
        if (disk_state != DISK_OK) {
          ...
          if (disk_state == DISK_CHANGED || disk_state == DISK_REMOVED) {
            ...
            change_init();
            fatops_init(0);
			bcis_status = 0xb; /* disk change (id mismatch) */
          } else
